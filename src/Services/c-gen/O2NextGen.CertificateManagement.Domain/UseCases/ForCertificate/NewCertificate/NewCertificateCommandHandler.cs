    ...
    IRequestHandler<NewCertificateCommand, NewCertificateCommandResult>
{
    ...
    public async Task<NewCertificateCommandResult> Handle(NewCertificateCommand request,
        CancellationToken cancellationToken)
    {
        var category =
            await _queryCategoryHandler.HandleAsync(new CategoryQuery(request.CategoryId), cancellationToken);
        ...
        return new NewCertificateCommandResult(
            ...
            string.Empty,
            0,
            DateTime.Now.AddDays(category.TimeLifeInDays).ConvertToUnixTime(),
            DateTime.Now.ConvertToUnixTime(),
            request.UserId,
            request.CustomerId,
            Guid.Empty.ToString(),
